...
public class Scanner {
	...
	public Scanner(BufferedReader input) {
		...
		ssym['~'] = Symbol.bitnot;
		ssym['!'] = Symbol.logicnot;
		ssym['%'] = Symbol.mod;
		
		// 设置保留字名字,按照字母顺序，便于折半查找
		word = new String[] {"begin", "call", "const", "do","else", "end","for", "if",
			"odd", "procedure", "read", "then", "var", "while", "write","writech"};
		...
		wsym[14] = Symbol.writesym;
		wsym[15] = Symbol.writeCh;
	...
	void matchOperator() {
		...
		case '/' : {
			...
			if (ch == '=') {
				sym = Symbol.slashEql;		//	/=
				getch();
			} else if(ch=='*')/*添加注释*/		//TODO:要加else,否则/=*会被识别为注释
			{
				getch();
				while(true)
				{
